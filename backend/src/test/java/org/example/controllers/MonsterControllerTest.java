...
/**
 * Controller test for MonsterController 
 */
@SpringBootTest
...
class MonsterControllerTest {
    ...
    class Update {
        ...
        @Test
        @WithMockUser(username = "user")
        void updatingAMonsterThatDoesNotExist_currentlyThrowsUnhandled_ratherThanReturning404() throws Exception {
            ...
            // in the app) when the id doesn't exist. GET-by-id explicitly avoids
            // this with its own ResponseStatusException(NOT_FOUND) -
            // update()/delete() don't. Worth a small fix (an @ExceptionHandler
            // mapping DaoException to 404) - flagging it here rather than
            // changing it unasked.
            //
            // In a real deployed app an uncaught exception like this becomes a
            // generic 500 response - the servlet container catches it. MockMvc's
            // test dispatcher doesn't hide it the same way though: without a
            // @ControllerAdvice to handle it, MockMvc re-throws the exception
            // straight out of perform() (wrapped in a ServletException) instead
            // of turning it into a response, so that's what this test checks for.
            ...
            assertInstanceOf(DaoException.class, thrown.getCause());
            assertEquals("Monster not found.", thrown.getCause().getMessage());
        }
    }
